...
_SENSITIVE_KEY_RE = re.compile(r"(token|secret|password|passwd|api[_-]?key|authorization|cookie|credential)", re.I)
    ...
    if isinstance(value, dict):
        ...
        for key, item in value.items():
            if _SENSITIVE_KEY_RE.search(str(key)):
                ...
                redacted[str(key)] = redact_sensitive(item, limit=limit)
        return redacted
    if isinstance(value, list):
        return [redact_sensitive(item, limit=limit) for item in value[:50]]
    if isinstance(value, str):
        return _SECRET_VALUE_RE.sub(r"\1=[REDACTED]", value)[:limit]
    return value
...
def evidence_ref_from_tool_call(
    *,
    ...
    result_text = _text_result(safe_result)
    if not result_text:
        return None
    source, trust, validates = classify_tool_evidence(tool_name, safe_args if isinstance(safe_args, dict) else {}, safe_result, status=status)
    locator, artifact_path = _first_locator(safe_args if isinstance(safe_args, dict) else {}, result_text)
...
def record_tool_evidence(
    *,
    ...
    """Build a post-tool evidence record.

    Phase 3 keeps persistence outside this module. Phase 4 will wire the
    returned ref into ``post_tool_call`` and ``SupergoalStore``.
    """
    return evidence_ref_from_tool_call(
        goal_run_id=goal_run_id,
        turn_id=turn_id,
        tool_name=tool_name,
        args=args,
