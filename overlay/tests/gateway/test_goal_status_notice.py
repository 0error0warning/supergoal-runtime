...
from hermes_cli.goals import CONTINUATION_PROMPT_TEMPLATE, SUPERGOAL_CONTINUATION_PROMPT_TEMPLATE
...
def _supergoal_continuation_event(source, goal="finish the big task"):
    return MessageEvent(
        text=SUPERGOAL_CONTINUATION_PROMPT_TEMPLATE.format(goal=goal),
        message_type=MessageType.TEXT,
        source=source,
    )
...
def test_clear_goal_pending_continuations_removes_supergoal_continuations():
    """Regression: /supergoal pause/clear must cancel queued self-continuations too."""
    runner = GatewayRunner.__new__(GatewayRunner)
    adapter = FakeAdapter()
    adapter._pending_messages = {}
    runner._queued_events = {}
    ...
    adapter._pending_messages[session_key] = _supergoal_continuation_event(source)
    runner._queued_events[session_key] = [
        normal_event,
        _supergoal_continuation_event(source, goal="second continuation"),
    ]
