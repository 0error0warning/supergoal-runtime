...
class TestHandleFunctionCall:
    ...
    def test_no_listener_skips_post_and_transform_emit_but_records_supergoal_evidence(self):
        """No plugin listeners should not suppress the supergoal evidence ledger."""
        with (
            patch("model_tools.registry.dispatch", return_value='{"ok":true}'),
            patch("hermes_cli.plugins.has_hook", return_value=False),
            patch("hermes_cli.plugins.invoke_hook") as mock_invoke_hook,
            patch("hermes_cli.supergoal.evidence.record_tool_evidence") as mock_record_evidence,
        ):
            result = handle_function_call(
                "web_search",
                {"q": "test"},
        ...
        mock_record_evidence.assert_called_once()
