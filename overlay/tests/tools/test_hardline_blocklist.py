...
def test_supergoal_full_auto_bypasses_recoverable_dangerous_approval(clean_session, monkeypatch, _isolate_hermes_home):
    from hermes_cli.goals import GoalManager, save_goal

    sid = "super-full-auto-approval-sid"
    mgr = GoalManager(session_id=sid)
    state = mgr.set("autonomous maintenance", mode="supergoal")
...
def test_supergoal_full_auto_cannot_bypass_hardline(clean_session, monkeypatch, _isolate_hermes_home):
    from hermes_cli.goals import GoalManager, save_goal

    sid = "super-full-auto-hardline-sid"
    mgr = GoalManager(session_id=sid)
    state = mgr.set("autonomous maintenance", mode="supergoal")
    state.permission_mode = "full_auto"
    save_goal(sid, state)
